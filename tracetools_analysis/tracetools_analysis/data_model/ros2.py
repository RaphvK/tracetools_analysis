...
class Ros2DataModel(DataModel):
    ...
    def add_rcl_publish_instance(
        self, publisher_handle, timestamp, message, cpu_id, pid, tid,
    ) -> None:
    ...
    def add_callback_instance(
        self, callback_object, timestamp, duration, intra_process, cpu_id_start, cpu_id_end, pid, tid
    ) -> None:
        self._callback_instances.append({
            ...
            'cpu_id_start': cpu_id_start,
            'cpu_id_end': cpu_id_end,
            'pid': pid,
            'tid': tid,
        })
